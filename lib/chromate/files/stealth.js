...
(() => {
  ...
  // 5) Simulation d’un contexte WebGL minimal
  ...
  HTMLCanvasElement.prototype.getContext = function (type, ...args) {
    if (['webgl', 'experimental-webgl', 'webgl2'].includes(type)) {
      ...
      // IMPORTANT : faire en sorte que notre fakeContext ressemble à un vrai WebGLRenderingContext
      if (window.WebGLRenderingContext && typeof window.WebGLRenderingContext === 'function') {
        Object.setPrototypeOf(fakeContext, window.WebGLRenderingContext.prototype);
      }
      return fakeContext;
    }
  ...
  // 6) Correction des dimensions d’images pour éviter le retour de 16x16
  ...
  // 7) Randomisation pour varier l'injection et rendre la détection plus difficile
  (function () {
    const randomSuffix = Math.random().toString(36).substring(2);
    document.documentElement.setAttribute('data-stealth', randomSuffix);
  })();

  // 8) Masquer la modification des fonctions natives (exemple avec toString)
  (function () {
